			...
			<el-table v-loading="loading" :data="groupMemberList">
				<el-table-column label="头像" align="center" prop="headImage" width="100">
					<template #default="scope">
						<image-preview v-if="scope.row.headImage" :src="scope.row.headImage" :width="50" :height="50" />
					</template>
				</el-table-column>
				...
				<el-table-column label="标签" align="center" prop="label">
					<template #default="scope">
						<el-tag v-if="scope.row.isManager" class="tag" type="primary">管理员</el-tag>
						<el-tag v-if="scope.row.isMuted" class="tag" type="warning">禁言中</el-tag>
					</template>
				</el-table-column>
