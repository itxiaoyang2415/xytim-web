          ...
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="消息状态" prop="status">
              <el-select v-model="queryParams.status" clearable>
                <el-option v-for="dict in im_message_status" :key="dict.value" :label="dict.label"
                  :value="dict.value" />
              </el-select>
            </el-form-item>
            <el-form-item>
      ...
      <el-table v-loading="loading" :data="privateMessageList">
        <el-table-column label="发送用户" align="center" prop="sendUserName">
          <template #default="scope">
            ...
            <span v-if="scope.row.sendCompanyName" class="company-tag">@{{ scope.row.sendCompanyName }}</span>
            <el-tag v-if="scope.row.isSendBanned" class="tag" title="该用户已被封禁" round type="danger" size="small"
              effect="dark">封</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="接收用户" align="center" prop="recvUserName">
          <template #default="scope">
            <span>{{ scope.row.recvUserName }}</span>
            <span v-if="scope.row.recvCompanyName" class="company-tag">@{{ scope.row.recvCompanyName }}</span>
            <el-tag v-if="scope.row.isRecvBanned" class="tag" title="该用户已被封禁" round type="danger"
              size="small">封</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" @click="handleDetail(scope.row)"
              v-hasPermi="['im:privateMessage:query']">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
